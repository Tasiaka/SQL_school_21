WITH visits AS
(
	SELECT person.name, person.gender, person.id AS person_id, pizzeria.name AS pizzeria_name, menu.pizza_name
	FROM person
...
((SELECT pizzeria_name FROM women WHERE pizzeria_name NOT IN (SELECT pizzeria_name FROM men)) 
UNION
(SELECT pizzeria_name FROM men WHERE pizzeria_name NOT IN (SELECT pizzeria_name FROM women))) as sasha
